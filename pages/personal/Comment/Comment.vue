<template>
	<view>
		...
		<view class="order">
			<u-rate :count="count" v-model="mark"></u-rate>
			<textarea type="text" v-model="comment" placeholder-class="line" placeholder="评价" />
			</view>

		<u-button @click="addComment()" style="color: white; background-color: #339966; position: absolute; bottom: 0px; width: 100%;">发布</u-button>
	...
	export default {
		...
		methods: {
			...
			onBackPress(e) {
			 ...
			 addComment(){
			      addComment(user.state.userId,this.goodsDetial.id,this.comment, this.mark).then(res => {
