	...
	<view class="wrap">
		...
		<u-form :model="model" :rules="rules" ref="uForm" :errorType="errorType">
			<u-form-item :leftIconStyle="{color: '#888', fontSize: '32rpx'}" label-width="120" :label-position="labelPosition"
			 label="名称" prop="title">
				<u-input :border="border" :placeholder="goods.title" v-model="model.title" type="text"></u-input>
			</u-form-item>
			<!-- <u-form-item :label-position="labelPosition" label="简介" prop="sex">
			...
			<u-form-item :label-position="labelPosition" label="简介" prop="abstract">
				<u-input type="textarea" :border="border" :placeholder="goods.abstract" v-model="model.abstract" />
			</u-form-item>
			<u-form-item :label-position="labelPosition" label="押金" prop="deposit">
				<u-input :border="border" type="number" v-model="model.deposit" :placeholder="goods.rental"></u-input>
			</u-form-item>
			<u-form-item :label-position="labelPosition" label="租金" label-width="150" prop="rental">
				<u-input :border="border" type="number" v-model="model.rental" :placeholder="goods.deposit"></u-input>
			</u-form-item>
		...
		<u-tabbar v-model="navcurrent" :show="navshow" :bg-color="navbgColor" :border-top="borderTop" :list="navlist" :inactive-color="inactiveColor"
...
<script>
	import {getGoodsInfo} from '@/api/goodsDetail.js'
	import {
		addUserGoods
	} from '@/api/user.js'
	...
	export default {
		...
		mounted() {
			getGoodsInfo(this.id).then(res =>{
				this.goods = res.data
				this.goods.rental = Number(this.goods.rental)
				this.goods.deposit = Number(this.goods.deposit)
				console.log(this.goods)
			}).catch(err =>{
				console.log(err)
			})
				...
				this.$refs.uForm.validate(valid => {
					...
					if (valid) {
						...
						this.photo = {
							...
							image: this.image,
							firmId: user.state.userId
						};
						this.requestParameters = Object.assign({}, this.photo, this.model)
						console.log('requestParam', this.requestParameters)
						console.log(addUserGoods)
