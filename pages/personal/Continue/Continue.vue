<template>
	<view style="background-color: #f3f4f6;">
		...
		<u-button @click="continueOrder" style="color: white; background-color: #339966; position: absolute; bottom: 0px; width: 100%;">续租</u-button>
	...
	export default {
		...
		methods: {
			...
			continueOrder(){
			      ...
			      continueOrder(this.order.id, this.reletTime, Number(this.goodsDetial.rental)*this.reletTime).then(res => {
			        console.log("res-order",res.data)
					uni.navigateTo({
						url: '../Pay/ContinuePay?id=' + this.order.id,
		...
		continueOrder() {
			console.log("update-order", this.order)
			continueOrder(this.order.id, this.order.reletTime, this.goodsDetial.rental * this.reletTime).then(res => {
				console.log("res-order", res.data)
				// this.$router.push({name: 'pay',
				//   params: {goodsId: this.goodsDetial.id,startTime: this.startTime, endTime: this.endTime, type: this.type, }})
			}).catch(error => {
